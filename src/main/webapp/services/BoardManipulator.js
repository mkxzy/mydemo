...
angular.module('demoApp').factory('BoardManipulator', ['$resource',function ($resource) {

    var base_url = "http://localhost:9222/backlog";
    ...
    addCardToColumn: function (board, column, cardTitle, details) {
      angular.forEach(board.columns, function (col) {
        if (col.name === column.name) {
            var card = new Card(cardTitle, column.name, details);
            col.cards.push(card);
            var Backlog = $resource(base_url, {});
            //Backlog.save({}, card);
        }
      });
    },
